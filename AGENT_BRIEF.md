...
**`series[].color` 預設留空**，`chartkit` 會照上表順序給（`PALETTE[i % len]`）。**只有要指定誰是主角時才寫死**——例如兩條線裡想讓第二條當主角。2026-08-05 那期九條序列 100% 手寫了 color，值全部等於程式本來就會給的預設，等於白工。
...
- **`series[].color` 明訂預設留空。** `chartkit` 的 `s.color or PALETTE[i % len]` fallback 一直都在，但兩份文件都沒記載，結果 2026-08-05 那期九條序列 100% 手寫了 color、值全部等於程式本來就會給的預設——**fallback 路徑從未被走到，等於白工，還讓換配色變成逐條手改**。
