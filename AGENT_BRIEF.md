...
1. **數字型**（上次觸及這個水位、十年第幾分位、第 N 次出現）——**一律從自己的序列算出來，並寫出該次的日期**。序列就在 JSON 裡，`上一次 60 日波動率站上 93 分位是 2020-05` 這種話是算得出來的，**不要用記憶回想**。算出來的那個時點，**若落在圖的窗口內就加一個 `marker` 標在圖上**（如 08-06 黃金波動率那張的「疫情期高點 1.73」——那就是標準做法）。
