...
### 2026-08-12 · 第 23 版（三種新圖型同日首次上線，互動軌兩個缺陷：gauge 讓整頁掛掉、grouped_bar 柱子對不上標籤）
...
| **當時已知的風險** | 參考刻度改用第二條 gauge 疊色段，**與靜態軌不是同一套實作**（靜態是 zorder 疊圖），仍屬雙軌兩份；`ref_label` 在網頁上併進 series 名稱、在 PNG 上貼在刻度旁，位置不同。`stacked_bar` 沿用 `barWidth: 62%` 是對的（堆疊同位置），但**那條路徑同樣沒有被跑過**；`stacked_bar`／`pct_stacked_bar`／`heatmap` 仍是零使用，且**仍然沒有任何機制把 option 餵進 ECharts 驗證** |
...
- **同一天、同一個根因，抓到兩個**：gauge 的參考線讓整頁掛掉（硬失敗，一眼看得出來），grouped_bar 的柱子與標籤整組錯位（**軟失敗，圖照樣畫出來、數字也對，只有位置錯**）。後者更危險——它不會丟例外、不會有任何紅字，只有人去看網頁才會發現，而 PNG 是對的。
